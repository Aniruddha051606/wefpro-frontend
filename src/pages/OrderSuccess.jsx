...
const OrderSuccess = () => {
  ...
  const { orderId, amount, customerName } = location.state || {}; // Get data passed from Checkout
    ...
    <div className="min-h-screen bg-stone-50 flex items-center justify-center p-6 font-sans">
      <div className="bg-white p-8 md:p-12 rounded-3xl shadow-xl max-w-lg w-full text-center space-y-8 animate-fade-in">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Link to={`/track?id=${orderId}`} className="flex items-center justify-center gap-2 bg-black text-white py-3 rounded-xl font-bold hover:bg-stone-800 transition">
                <Package size={18} /> Track Order
            </Link>
            <Link to={`/invoice/${orderId}`} className="flex items-center justify-center gap-2 bg-stone-100 text-stone-900 py-3 rounded-xl font-bold hover:bg-stone-200 transition border border-stone-200">
                <FileText size={18} /> Get Invoice
            </Link>
        </div>

        <Link to="/" className="block text-stone-400 hover:text-red-600 text-sm transition">
            Return to Home
        </Link>
      </div>
    </div>
